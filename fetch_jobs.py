...
def infer_type(title):
    ...
    # 기간제/초빙 판정보다 먼저 (예: 비상임 이사 초빙 → 임원)
    # 단, 감사관·감사담당은 실무직이라 제외 (예: 법무감사관 → 임기제 유지)
    # 주의: "이사" 단독 매칭은 "아이사랑" 같은 일반 단어 오탐 유발 → 뒤에 한글이 오면 제외 (2026-09-23 수정)
    if any(k in t for k in ["상임", "비상임", "임원", "이사장", "사외이사", "사장", "원장", "관장"]):
        return "임원"
    if re.search(r"이사(?![가-힣])", t):
        return "임원"
    if "감사" in t and not any(k in t for k in ["감사관", "감사담당"]):
        return "임원"
    # 일용직 계열 (스킬 규칙: 단기노무/한시인력/일용/계절 → "정규직" 오표기 금지)
    # "단기" 포괄 (단기노무원·단기계약근로자 모두 커버 — 2026-09-23 수정, alio-305291 오표기 실측)
    # 단, "한시임기제"는 공무원 임기제 유형이라 제외 (예: 한시임기제공무원 7호 → 임기제 유지)
    if "한시임기제" not in t and any(k in t for k in ["단기", "한시인력", "일용직", "일용", "계절근로", "한시적", "한시"]):
        return "일용직"
...
THREAD_SLOTS = [s.strip() for s in os.environ.get("THREAD_SLOTS", "08:00,11:00,14:00,18:00,21:00").split(",") if s.strip()]
# 11:00은 콘텐츠형 고정 슬롯 (공고 4 + 콘텐츠 1, 2026-09-26 5슬롯제)
CONTENT_SLOT = os.environ.get("THREAD_CONTENT_SLOT", "11:00")
